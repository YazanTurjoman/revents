...
const EventDetailedChat = ({ eventId }) => {
  ...
  function handleCloseReplyForm() {
    setShowReplayForm({ open: false, commentId: null });
  }

  useEffect(() => {
    getEventChatRef(eventId).on('value', (snapchat) => {
      if (!snapchat.exists()) return;
      dispatch(
        listenToEventChat(firebaseObjectToArray(snapchat.val()).reverse())
      );
    });
    return () => {
              ...
              <Comment key={comment.id}>
                ...
                <Comment.Content>
                  ...
                  <Comment.Actions>
                    <Comment.Action
                      onClick={() =>
                        setShowReplayForm({ open: true, commentId: comment.id })
                      }
                    >
                      Reply
                    </Comment.Action>
